...
impl TyCtxt {
    // TODO: Use new indexing system and keep regions in mind.
    pub fn determine_impl_generic_solve_order2(&self, def: Obj<ImplDef>) {
        let s = &self.session;
...
impl InferCx<'_> {
    ...
    pub fn relate_ty_and_trait(
        ...
        for &candidate in rhs.value.def.r(s).impls.read().iter() {
            match self.relate_ty_and_impl_no_fork(lhs, candidate, rhs, binder) {
                Ok(resolution) => {
